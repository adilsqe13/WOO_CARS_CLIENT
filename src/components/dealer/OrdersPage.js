...
export default function Homepage() {
    ...
    const getOrders = async () => {
        try {
            const response = await fetch(`${apiUrl}/api/dealer/get-orders`, {
                method: 'GET',
                headers: {
    ...
    const handleConfirm = async (orderId) => {
        try {
            const response = await fetch(`${apiUrl}/api/dealer/order-confirm`, {
                method: 'PUT',
                headers: {
                    "Content-Type": "application/json",
                    "auth-token": token
        ...
        <>
            <section style={{ backgroundColor: "#ffffff" }}>
                <div className="container py-5">
                    <h1 className='mt-3'>Deals <small className='h6 text-secondary'>&nbsp; &nbsp; {orders === null ? 0 : orders.length} - deal</small></h1>
                    <div className="row justify-content-center mb-3 mt-4">

                        <h5 className='text-danger text-align-center dfjcac'>{orders === null ? '' : orders.length === 0 ? 'No Deals' : ''}</h5>
